...
#checkname:	rain_noamount6h
#signature:	obs;RR_6,WW;;
# Konsistenskontroll_Check
# Siva Navaratnam  Wed Jun 11 11:21:07 2003
# Endret av Gabriel Kielland 12. april 2005:
# Sjekken slår ikke til når RR_6 mangler, bare når det er meldt tørt. 
# Endret av Bjørn Nordin 22. august 2006:
# Sjekken gikk for langt tilbake for WW(20-27).
...
#tolererer minst en ikke manglende observasjon:
	...
	my @retvector;
	for (my $i=0; $i < @WW; $i++) {
		if ($RR_6[0]==-1 && (($WW[$i] >= 20 && $WW[$i] <= 27 && $obs_timeoffset[$i] >= -350) || ($WW[$i] >= 50 && $WW[$i] <= 99))) {
			$flag = 3;
