...
#language        : 1
#checkname       : snow_relativelywarm
#signature       : obs;X,Y;;|meta;R1,R2,R3,R4,R5,R6,R7,R8,R9,R10,R11,R12,R13,R14;;
#skript          : se nedenfor
...
sub check {
  ...
  if ((($X[0] == $R1[0]) || ($X[0] == $R2[0]) || ($X[0] == $R3[0]) || (($X[0] <= $R5[0]) && ($X[0] >= $R4[0])) || ($X[0] == $R6[0]) || ($X[0] == $R7[0]) || (($X[0] <= $R9[0]) && ($X[0] >= $R8[0])) || (($X[0] <= $R11[0]) && ($X[0] >= $R10[0])) || ($X[0] == $R12[0]) || ($X[0] == $R13[0])) && ($Y[0] > $R14[0]))
