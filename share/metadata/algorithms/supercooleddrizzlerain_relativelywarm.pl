...
#language        : 1
#checkname       : supercooleddrizzlerain_relativelywarm
#signature       : obs;X,Y;;|meta;R1,R2,R3,R4,R5,R6,R7,R8;;
#skript          : se nedenfor
...
sub check {
  ...
  if ((($X[0] == $R1[0]) || ($X[0] == $R2[0]) || ($X[0] == $R3[0]) || ($X[0] == $R4[0]) || ($X[0] == $R5[0]) || ($X[0] == $R6[0]) || ($X[0] == $R7[0])) && ($Y[0] > $R8[0]))
