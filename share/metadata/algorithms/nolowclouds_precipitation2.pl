...
#language        : 1
#checkname       : nolowclouds_precipitation2
#signature       : obs;X,Y;;|meta;R1,R2,R3,R4,R5;;
#skript          : se nedenfor
...
# Denne kontrollen returnerer verdien 1 ved ok kontroll og verdien 2 ved feil.
...
sub check  {
  ...
  if ($X[0] == $R1[0] && (($Y[0] <= $R3[0] && $Y[0] >= $R2[0]) || ($Y[0] <= $R5[0] && $Y[0] >= $R4[0])))
  {
	$flag = 3;
    }
