...
#checkname:		windmaxmean3h
#signature:		obs;FX,FF;;|meta;FX_C;;
# Konsistenskontroll_Check
# Feilsituasjon: FF(n) > FX
# Tidspunkter, FX (0): 03,09,15,21 (Mellomobser)
# Tidsperiode, FF(0,1,2) : 3 timer bakover fra Mellomobs.
# Bjørn Harald Nordin 2006 04 04.
...
#tolererer minst en ikke manglende observasjon:
    ...
    for (my $i=0; $i < $N; $i++) {    	
        ...
        #####  av disse som gjelder for de siste 3 timene.
        $FFflag[$i]=1;
      }
...
#Først FX som gjelder for nå-tidspunkt. (kl.03,09,15,21 dette styres av checks.active)
