...
#language        : 1
#checkname       : precip_weather2
#signature       : obs;RR_12,V1,V2,V3,V4,V5,V6,V7;;0,-720;;
#skript          : se nedenfor
...
#                      
#  ---- RR_12 ----               Versjon 1.04 15/3-2004, RR_12
#  ----      ----     
...
## obs;RR_12,V1,V2,V3,V4,V5,V6,V7;;0,-720;; 
## -720 angir alle verdier 12 timer tilbake.


    for (my $i=0; $i < $N; $i++) {    	
        #####  Initialiserer flaggverdiene til kodene V1,...,V7 (været siden 
        #####  siste hovedobservasjon) for alle innkommende av disse som gjelder
        #####  for de siste 12 timene.
        $V1flag[$i]=1;
	$V2flag[$i]=1;
	$V3flag[$i]=1;
      }

#for V4,...V7 må vi passe på at vi ikke teller med verdien stemplet -720 minutter tilbake,
#for den gjelder for enda en dag tilbake. Men hvis siste tidsangivelse i obs_timeoffset > -720
#er saken grei.
    for (my $i=0; $i < $M; $i++) { 
	$V4flag[$i]=1;
	$V5flag[$i]=1;
...
####    Tilfelle 1    #######
#####################################################################
####  RR_12[0]==-1 (ingen nedbør) og V1/V2/V3/V4/V5/V6/V7 = nedbørsymbol
 

if ($RR_12[0]>-1.01 && $RR_12[0]<-0.99) 
 {
  for (my $i=0; $i < $N; $i++) {
...
} # end if($RR_12[0]>-1.01 && $RR_12[0]<-0.99) 



 ########   Tilfelle 2 og tilfelle 3 slår vi sammen.  ############
#### RR_12[0]=0.0 (nedbør, men ikke målbar) men det finnes ikke nedbørsymbol i V1/V2/V3/V4/V5/V6/V7
  if ($RR_12[0]>-0.01) {
...
#---------Tester for hver parameter (RR_12,V1,....., V7) om de har 
#---------kommet inn, de skal isåfall flagges med 1 eller 3, alt ettersom
#---------testene har slått ut.


#Først RR_12 som gjelder for nå-tidspunkt. (kl. 00 eller kl. 12, dette styres av checks.active)

	if ($RR_12_missing[0]==0) {               #Hvis verdien har kommet inn
	  push(@retvector, "RR_12_0_0_flag");
	  push(@retvector, $RR_12flag);
	}
