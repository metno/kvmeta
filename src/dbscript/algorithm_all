...
sub insert_DB{
  ...
  my $sth = $dbh->prepare("select language from algorithms where checkname='$checkname'");
  $sth->execute;

  #my $sth = $dbh->do("select language from algorithms where checkname='$checkname'");
  ...
  if( $len_language > 0 ){
	  ...
	  if($_ eq $language){ 
		  ...
		  my $sth = $dbh->prepare("UPDATE algorithms \
                                           ...
                                           WHERE language='$language' AND checkname='$checkname'");
                  $sth->execute;
                  $sth->finish;
                  return;
		}
  ...
  $sth = $dbh->prepare("INSERT INTO algorithms VALUES('$language', '$checkname', '$signature', '$script')");
  $sth->execute;
