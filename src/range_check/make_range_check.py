...
with open(p_outputfilename,'w') as f:
  ...
  c_refst = conn.cursor()
  c_refst.execute("select distinct stationid from range_check_st_cat where st_group='" + p_st_group + "' and paramid=" + p_paramid)
  print("select distinct stationid from range_check_st_cat where st_group='" + p_st_group + "' and paramid=" + p_paramid)
