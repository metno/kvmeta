...
for l_st, in c_st_cat:
    c_select.execute("select count(*) from range_check_data where stationid=" + str(l_st) + " and paramid=83")
    count, = c_select.fetchone()
    print( "count=" + str(count) + ": l_st= " + str(l_st) )
    if ( count == 0 ):
        print( "count == 0: l_st= " + str(l_st) )
        c_insert_data.execute("INSERT INTO range_check_data SELECT stationid, month, 83, amsl, st_low * " + l_factor + ", st_lowest * " + l_factor + ", calc_low * " + l_factor + ", calc_lowest * " + l_factor + ", no_of_years, st_high * " + l_factor + ", st_highest * " + l_factor + ", calc_high * " + l_factor + ", calc_highest * " + l_factor + ", countyid, hlevel, low * " + l_factor + ", lowest * " + l_factor + ", high * " + l_factor + ", highest * " + l_factor + ", edit_dato, st_group, 3,  edited_at from range_check_data where paramid=81 and stationid=" + str(l_st))
        conn.commit()
