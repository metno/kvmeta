...
$sth=$dbh->prepare("select max(physical_height) from sensor_info_default where measurement_methodid =217701");
$sth->execute;
...
$sth=$dbh->prepare("select stationid, hlevel, sensor, physical_height,sensor_info.fromtime from sensor_info where measurement_methodid=217701 and paramid=112") or die "Can't prep\n";
$sth->execute;
...
while (my @row = $sth->fetchrow()) {
    ...
    print_station_param($stationid,$paramid,$hlevel,$sensor,$physical_height,$fromtime );
...
sub print_station_param{
    my ($stationid,$paramid,$level,$sensor,$physical_height,$fromtime)=@_;
