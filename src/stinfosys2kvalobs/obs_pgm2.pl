...
if( $kvname eq "metno" ){
####### message_formatid <> 0
    $sth=$dbh->prepare("select stationid,message_formatid,kvalobsid from message_in where message_formatid <> 0 and kvalobsid <> 1 and totime is NULL") or die "Can't prep\n"; 
    $sth->execute;
    ...
    $sth=$dbh->prepare("select stationid,message_formatid,kvalobsid from message_in where message_formatid <> 0 and kvalobsid = 1 and sendtokvalobs is true and totime is NULL") or die "Can't prep\n"; 
    $sth->execute;
    while (my @row = $sth->fetchrow()) {
...
####### message_formatid=0
    $sth=$dbh->prepare("select stationid,kvalobsid from message_in where message_formatid=0 and kvalobsid <> 1 and totime is NULL") or die "Can't prep\n"; 
    $sth->execute;
    while (my @row = $sth->fetchrow()) {
        $NOT_METNO_station{$row[0]}=$row[1];
    }

    $sth=$dbh->prepare("select stationid,kvalobsid from message_in where message_formatid=0 and kvalobsid = 1 and sendtokvalobs is true and totime is NULL") or die "Can't prep\n"; 
    $sth->execute;
    while (my @row = $sth->fetchrow()) {
        $METNO_station{$row[0]}=$row[1];
    }

}elsif( $kvname eq "METNOSVV" ){
####### message_formatid <> 0
    $sth=$dbh->prepare("select stationid,message_formatid,kvalobsid from message_in where message_formatid <> 0 and kvalobsid <> 1 and totime is NULL") or die "Can't prep\n"; 
    $sth->execute;
    while (my @row = $sth->fetchrow()) {
...
my $base_sql="select stationid,paramid,hlevel,message_formatid,sensor,priority_message,anytime,array_to_string(hour,','),fromtime,totime from obspgm_h2";
...
while (my @row = $sth->fetchrow()) {
    ...
    if( ($kvname eq "metno") or ($kvname eq "METNOSVV") ){
        if( (( not exists $NOT_METNO{$row[0]}{$row[3]} ) and ( not exists $NOT_METNO_station{$row[0]} ) ) or ( exists $METNO{$row[0]}{$row[3]} ) or ( exists $METNO_station{$row[0]} ) ){
            print "$row[0]|$row[1]|$row[2]|$row[3]|$row[4]|$row[5]|$row[6]|$outhour|$row[8]|$totime\n";
        }
    }else{
        if( (exists $NOT_METNO{$row[0]}{$row[3]}) or (exists $NOT_METNO_station{$row[0]}) ){
            print "$row[0]|$row[1]|$row[2]|$row[3]|$row[4]|$row[5]|$row[6]|$outhour|$row[8]|$totime\n";
        }
    }
}
